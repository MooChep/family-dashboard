 ...
 *
 * Usage : GET /api/push/schedule (appelable depuis un cron système)
 * Fréquence recommandée : toutes les minutes via `crontab -e`:
 *   * * * * * curl -s http://localhost:3000/api/push/schedule
 */
...
async function sendPush(sub: SubRow, payload: PushPayload): Promise<void> {
  try {
    ...
    if (status === 410 || status === 404) {
      ...
      console.log(`[scheduler] subscription supprimée (${status}): ${sub.endpoint.slice(0, 60)}…`)
    } else {
