...
// GET /api/push/subscribe?endpoint=... — vérifie si CET appareil est abonné
export async function GET(request: NextRequest): Promise<Response> {
  ...
  if (endpoint) {
    const sub = await prisma.pushSubscription.findFirst({ where: { userId: session.user.id, endpoint } })
