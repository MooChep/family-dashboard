...
import { AccountManager } from '@/components/butin/AccountManager'
...
const scrollTo = (id: string) => {
  ...
  const commonProps = {
    categories, projets,
    onAdd: async (d: any) => { await fetch('/api/butin/categories', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(d) }); loadAll() },
    onEdit: async (id: string, d: any) => { await fetch(`/api/butin/categories/${id}`, { method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(d) }); loadAll() },
    onDelete: async (id: string) => { await fetch(`/api/butin/categories/${id}`, { method: 'DELETE' }); loadAll() },
    onAddProjet: async (n: string, t: any) => { await fetch('/api/butin/projets', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({name: n, targetAmount: t}) }); loadAll() },
    onEditProjet: async (id: string, n: string, t: any) => { await fetch(`/api/butin/projets/${id}`, { method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({name: n, targetAmount: t}) }); loadAll() },
    onReaffecterProjet: async (s: string, t: string) => { await fetch(`/api/butin/projets/${s}/reaffecter`, { method: 'PATCH', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({targetProjectId: t}) }); loadAll() },
    onAnnulerReaffectation: async (s: string) => { await fetch(`/api/butin/projets/${s}/reaffecter`, { method: 'DELETE' }); loadAll() },
  }
