...
import { ProjectList } from '@/components/butin/ProjectList'
import { MultiLineChart } from '@/components/butin/analyses/AnalysesCharts'
...
interface DashboardData {
  currentMonth: string
  summary: { revenus: number; depenses: number; butin: number; reste: number }
  totalFortune: number
...
export default function ButinDashboard(): ReactElement {
  ...
  async function loadData(): Promise<void> {
    try {
      ...
      const [dashRes, regulRes] = await Promise.all([
        fetch('/api/butin/dashboard'),
        fetch(`/api/butin/regul?month=${currentMonth}`),
      ])
  ...
  async function handleReaffecter(): Promise<void> {
    ...
    try {
      const res = await fetch(`/api/butin/projets/${reaffecterProjectId}/reaffecter`, {
        method: 'PATCH',
  ...
  return (
    <ButinLayout>
      <div className="flex flex-col gap-6 pt-12 md:pt-0">
        ...
        {regulDone === false && (
          <a href="/butin/regul" className="flex items-center gap-3 px-4 py-3 rounded-xl no-underline"
