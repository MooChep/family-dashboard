...
import { MonthSummary } from '@/components/butin/MonthSummary'
import { TransactionTable } from '@/components/butin/TransactionTable'
import { TransactionForm } from '@/components/butin/TransactionForm'
import { FixedChargesTable } from '@/components/butin/FixedChargesTable'
import { VariableChargesTable, type VariableChargeRow } from '@/components/butin/VariableChargesTable'
import { AllocationForm } from '@/components/butin/AllocationForm'
...
export default function MoisPage(): ReactElement {
  ...
  const loadAll = useCallback(async (): Promise<void> => {
    ...
    try {
      const [txRes, chargesRes, varRes, catsRes, projetsRes, allocRes, allTxRes, budgetRes] =
        await Promise.all([
          fetch('/api/butin/transactions?month=' + currentMonth),
          fetch('/api/butin/charges-fixes?month=' + currentMonth),
          fetch('/api/butin/charges-variables?month=' + currentMonth),
          fetch('/api/butin/categories'),
          fetch('/api/butin/projets'),
          fetch('/api/butin/allocations?month=' + currentMonth),
          fetch('/api/butin/transactions'),
          // Fetch du budget du mois — on ignore l'erreur si pas encore créé (404 = pas de budget)
          fetch('/api/butin/budget/' + currentMonth).then((r) => r.ok ? r.json() : null),
        ])
  ...
  async function handleSaveTransaction(f: {
    ...
    if (isProjectCat && !editingTransaction) {
      ...
      const res = await fetch(`/api/butin/projets/${projet.id}/depense`, {
        method: 'POST',
      ...
      const url = editingTransaction
        ? '/api/butin/transactions/' + editingTransaction.id
        : '/api/butin/transactions'
  ...
  async function handleDeleteTransaction(id: string): Promise<void> {
    const res = await fetch('/api/butin/transactions/' + id, { method: 'DELETE' })
    if (!res.ok) throw new Error('Erreur suppression')
    await loadAll()
  }

  async function handleTogglePointage(id: string): Promise<void> {
    await fetch('/api/butin/transactions/' + id + '/pointage', { method: 'PATCH' })
  ...
  async function handleUpdateFixed(categoryId: string, estimated: number): Promise<void> {
    await fetch('/api/butin/charges-fixes', {
  ...
  async function handleUpdateVariable(categoryId: string, estimated: number): Promise<void> {
    await fetch('/api/butin/charges-variables', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ month: currentMonth, categoryId, estimated }),
    })
    await loadAll()
  ...
  async function handleSaveAllocations(allocs: { projectId: string; percentage: number }[]): Promise<void> {
    const res = await fetch('/api/butin/allocations', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
...
>      <div className="flex flex-col gap-6">
        {isLoading ? (
          ...
          <>
            ...
            <div className="rounded-xl overflow-hidden" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)' }}>
              <div className="px-5 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
                <h2 className="text-base font-semibold" style={{ color: 'var(--text)', fontFamily: 'var(--font-display)' }}>Affectation butin</h2>
