...
export function DictionaryView() {
  ...
  async function loadAll() {
    ...
    try {
      const [iRes, aRes] = await Promise.all([
        fetch('/api/popote/ingredients'),
        fetch('/api/popote/aisles'),
      ])
  ...
  async function handleSave(data: IngredientFormData) {
    if (editing) {
      const res  = await fetch(`/api/popote/ingredients/${editing.id}`, {
        method:  'PATCH',
  ...
  async function handleDelete(item: IngredientWithAisle) {
    const res  = await fetch(`/api/popote/ingredients/${item.id}`, { method: 'DELETE' })
    if (res.status === 409) {
  ...
  async function handleMergeConfirm() {
    ...
    await fetch(`/api/popote/ingredients/${merge.sourceId}/merge`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
