...
export function AislesConfig() {
  ...
  async function load() {
    setLoading(true)
    try {
      const res  = await fetch('/api/popote/aisles')
  ...
  async function move(index: number, direction: 'up' | 'down') {
    ...
    setAisles(next)
    await fetch('/api/popote/aisles/reorder', {
      method:  'POST',
  ...
  async function handleRename(id: string) {
    ...
    setEditingId(null)
    await fetch(`/api/popote/aisles/${id}`, {
      method:  'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function handleDelete(aisle: Aisle) {
    const res = await fetch(`/api/popote/aisles/${aisle.id}`, { method: 'DELETE' })
    if (res.status === 409) {
