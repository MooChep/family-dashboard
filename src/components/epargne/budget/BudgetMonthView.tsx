...
const GROUPS: {
  ...
  { type: 'PROJECT', label: "Projets d'épargne", description: "Chaque ligne définit l'objectif du projet. Le total devient le montant cible à la sauvegarde.", color: 'var(--accent)'  },
]
...
export function BudgetMonthView({ month }: BudgetMonthViewProps): ReactElement {
  ...
  const loadData = useCallback(async (): Promise<void> => {
    ...
    try {
      const [budgetRes, catRes] = await Promise.all([
        fetch(`/api/epargne/budget/${month}`),
      ...
      for (const entry of localEntries) {
        if (entry.isNew && !entry.isDeleted) {
          ...
          if (entry.recurrence !== 'NONE') {
            ops.push(fetch('/api/epargne/budget/lines', {
              method: 'POST',
          ...
          ops.push(fetch('/api/epargne/budget/entries', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          // ── Suppression ───────────────────────────────────────────────────
          ops.push(fetch(`/api/epargne/budget/entries/${entry.id}`, {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' },
  ...
  async function handleGenerate(): Promise<void> {
    ...
    try {
      const res = await fetch(`/api/epargne/budget/${month}`, { method: 'POST' })
  ...
  async function handleValidate(): Promise<void> {
    ...
    try {
      const res = await fetch(`/api/epargne/budget/${month}`, { method: 'PATCH' })
