...
export function CategoryManager({
  ...
  async function handleDelete(id: string): Promise<void> {
    setDeletingId(id)
    try { await onDelete(id) } catch (e) { alert(e instanceof Error ? e.message : 'Erreur') } finally { setDeletingId(null) }
  }
  ...
  function CategoryGroup({ title, items }: { title: string; items: Category[] }): ReactElement {
    return (
          ...
          {items.length === 0 ? (
            ...
            <div key={cat.id} className="flex items-center justify-between px-4 py-3" style={{ borderBottom: i < items.length - 1 ? '1px solid var(--border)' : 'none' }}>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="danger" size="sm" isLoading={deletingId === cat.id} onClick={() => handleDelete(cat.id)}>Supprimer</Button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-xl overflow-hidden" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)' }}>
          ...
          {tagsLoading ? (
            ...
            <p className="px-4 py-3 text-sm" style={{ color: 'var(--muted)' }}>
              {tagSearch ? 'Aucun résultat' : 'Aucun tag enregistré'}
            </p>
          ) : (
            filteredTags.map((row, i) => (
              <div
                ...
                style={{ borderBottom: i < filteredTags.length - 1 ? '1px solid var(--border)' : 'none' }}
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  {renamingTag === row.tag ? (
                    <input
                      ...
                      onKeyDown={(e) => handleRenameKey(e, row.tag)}
                      onBlur={() => saveRename(row.tag)}
                      className="px-2 py-0.5 rounded text-sm outline-none"
                      style={{ backgroundColor: 'var(--surface2)', border: '1px solid var(--accent)', color: 'var(--text)', fontFamily: 'var(--font-mono)', width: 160 }}
                    />
                  ) : (
                    <span
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex gap-2">
                {[{ value: CategoryType.INCOME, label: 'Revenu' }, { value: CategoryType.EXPENSE, label: 'Dépense' }].map((opt) => (
                  <button key={opt.value} type="button" onClick={() => setType(opt.value)} className="flex-1 py-2 rounded-lg text-sm transition-colors" style={{ backgroundColor: type === opt.value ? 'var(--accent)' : 'var(--surface2)', color: type === opt.value ? 'var(--bg)' : 'var(--text2)', border: '1px solid var(--border)' }}>
                    {opt.label}
                  </button>
                ))}
              </div>
