...
export function MonthSummary({ revenus, depenses, reste, allocationPercent }: MonthSummaryProps): ReactElement {
  ...
  // Taux de butin = (revenus - dépenses) / revenus * 100
  // = ce qui n'est pas dépensé, qu'il soit affecté ou non
  const tauxButin = revenus > 0 ? ((revenus - depenses) / revenus) * 100 : 0
  const tauxButinColor = tauxButin >= 20
    ? 'var(--success)'
    : tauxButin >= 10
    ? 'var(--warning)'
    : 'var(--danger)'
  ...
  return (
    <Card>
      <div className="flex flex-wrap justify-around gap-6">
        ...
        <SummaryItem
          label="Taux de butin"
          value={revenus > 0 ? `${Math.round(tauxButin)} %` : '—'}
          color={tauxButinColor}
        />
      </div>
    </Card>
  )
