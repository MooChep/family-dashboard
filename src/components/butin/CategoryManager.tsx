...
export function CategoryManager({
  ...
  async function loadTags() {
    setTagsLoading(true)
    try {
      const res = await fetch('/api/butin/tags')
  ...
  const handleDeleteTag = async (tag: string) => {
    ...
    try {
      await fetch('/api/butin/tags', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tag }) })
      await loadTags()
    } catch {}
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {(mode === 'all' || mode === 'projets') && (
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <h3 className="text-[10px] font-bold uppercase tracking-widest text-(--muted) font-mono">Projets de butin</h3>
            <div className="rounded-xl overflow-hidden border border-(--border) bg-(--surface)]">
      ...
      <Modal isOpen={isProjetModalOpen} onClose={() => setIsProjetModalOpen(false)} title="Projet de butin">
        <div className="flex flex-col gap-4 pt-2">
